...
ServerEvents.recipes((event) => {
  ...
  function combine(result, base, addition) {
    event.recipes.minecraft.smithing(result, base, addition);
    event.recipes.create.deploying([result], [base, addition]);
  }
  ...
  Object.entries(modules).forEach(([module, ingredient]) => {
    const low = `prettypipes:low_${module}_module`;
    const medium = `prettypipes:medium_${module}_module`;
    const high = `prettypipes:high_${module}_module`;

    combine(low, blank, ingredient);
    combine(medium, low, magma);
    combine(high, medium, sturdy);
  });
